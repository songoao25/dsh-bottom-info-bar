...
- **Provider & model detection** — Always shows provider and model separately, exactly as in the DSH LLM catalog (for example, current DSH displays `DeepSeek · V41-Flash` for the official V4.1 Flash release). The provider is bold; when a catalog model name repeats its provider prefix, only that duplicate prefix is removed from the model part. Model names and image capability are read from DSH's live catalog, so newly published models do not require a plugin code release.
- **Live balance** — Fetches real balance from DeepSeek's `/user/balance` API, refetches immediately when the bar opens/refreshes or the provider changes, then auto-refreshes every 60 s, and keeps the last known snapshot on failure so usage is never interrupted.
