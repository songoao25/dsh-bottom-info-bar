...
check('余额制服务商/模型 hover 显示当前插件版本', client.includes("'\\nPlugin version: ' + updateInfo.current"))
check('余额/订阅/账单制 hover 均显示当前插件版本（≥2 处）', (client.match(/'\\nPlugin version: ' \+ updateInfo\.current/g) || []).length >= 2)
check('client 只在有更新时显示新版本提醒文字', client.includes("'Update available'") && client.includes('updateInfo.available === true'))
check('更新标签提示语包含动态最新版本号', client.includes("title: 'Ask your agent to update this plugin to version ' + updateInfo.latest + '.'"))
