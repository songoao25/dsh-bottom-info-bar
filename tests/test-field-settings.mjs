...
// ---------- ④ density 落盘（修复重启即丢） ----------
{
  ...
  const route = s.stub.captured.route
  const compact = await invokeRoute(route, 'setInfoDensity', { density: 'compact' })
  check('setInfoDensity 即时生效', compact.body.infoDensity === 'compact', compact.body)
  ...
  check('setInfoDensity 可切回完整', set.body.infoDensity === 'full', set.body)
