...
// ---------- ① 读写往返 + 落盘持久（重启不丢） ----------
{
  ...
  check('getFieldConfig 默认全部显示', Object.entries(first.body.fields).every(([k,v]) => (['customText','mainTime','worldTime','subWindowWeek'].includes(k) ? v === false : v === true)) && Object.keys(first.body.fields).length >= 20, Object.keys(first.body.fields).length)
  check('getFieldConfig 默认 quotaDisplayMode=used', first.body.quotaDisplayMode === 'used', first.body.quotaDisplayMode)
...
// ---------- ② 损坏回退（显式 warn + 默认值） ----------
{
  ...
  check('损坏文件回退默认：字段全部显示', Object.entries(body.fields).every(([k,v]) => (['customText','mainTime','worldTime','subWindowWeek'].includes(k) ? v === false : v === true)), true)
...
// ---------- ⑦ __settingsInternals 单元（颜色归一化） ----------
{
  ...
  check('sanitize：默认结构含全部注册字段且颜色为 null', (() => {
    ...
    return d.version === 1 && d.infoDensity === 'full' && d.quotaDisplayMode === 'used' && Object.entries(d.fields).every(([k,v]) => (['customText','mainTime','worldTime','subWindowWeek'].includes(k) ? v === false : v === true)) && Object.values(d.colors).every((v) => v === null) && d.timeFormat && d.timeZones && typeof d.customText === 'string'
  })(), true)
}
