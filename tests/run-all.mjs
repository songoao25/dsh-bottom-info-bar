...
const cases = [
  ...
  ['test-info-bar-rhythm（两行节奏：确定高度收合 / 无 fr 轨道 / 无行间空隙）', ['tests/test-info-bar-rhythm.js'], join(root), process.execPath],
  ...
  ['test-minimax-token-plan（MiniMax Token Plan 多桶最紧聚合 + Subscription Key 错误码降级）', ['tests/test-minimax-token-plan.js'], join(root), process.execPath],
  ['test-quota-display-mode（订阅窗口百分比方向：默认 remaining / 切换 used / 落盘与回滚）', ['tests/test-quota-display-mode.js'], join(root), process.execPath],
