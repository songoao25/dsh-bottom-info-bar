...
> ⚠️ **本文是历史存档，不是现行发布流程，请勿照做。**
>
> 文中第 3、5 步的「发布准备提交：版本号 1.8.0→1.9.0」「tag v1.9.0 → `gh release create`」属于
> **手工改版本号 + 手工打标签**，该做法已随发布机制改革而**废弃**——它正是 2026-09-11
> v2.0.0 误发事故的成因（详见 `MEMORY.md`）。
>
> **现行流程见 `AGENTS.md` 的「发布机制」**：版本号 / CHANGELOG / tag 全部由 Release Please
> 自动完成，**严禁手工修改版本文件**；合并「发布 PR」是唯一的人工闸门。
>
> 保留本文仅为存档 v1.9.0 / v1.9.1 当时的发布过程与产物。
