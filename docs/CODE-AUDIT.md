...
| P0-3 | `host.js:21`（对照 `:108`）、`:320`–`:322` | ~~`DATA_DIR` 不认 `DSH_HOME`~~ → 用户拍板：网页端与桌面端**分开**（各归各，不合并、不迁移）。已落地：`DATA_DIR` 改走 `dshHomeDir()`（无值时与原来逐字相同，老用户不搬家）；登录态是「人」的身份只读不动；opencode 登录按序试读（环境变量 > 平台共享目录 > 历史路径，win32 未实测已注明） | 已修 |
...
| **E（本次已含）** | P0-3 路径唯一入口（用户拍板分开）· P2 收尾 | 已发布 |
