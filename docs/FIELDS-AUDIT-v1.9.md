...
> ⚠️ **历史快照（2026-08-28）：本文描述的是当时的行为，其中「简洁模式只显示优先窗口」「某字段只在完整模式出现」一类结论已于 2026-09-26 全部作废 —— 现在模式只决定原生统计行参不参与，字段显隐只由字段开关决定。以 `docs/DECISIONS-SETTINGS-REDESIGN.md` 第 6 节为准。** 保留原文是为了留住决策脉络，请勿据此理解现行为。
