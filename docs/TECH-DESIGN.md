...
> 历史快照（只读，2026-09-26 审计加注）：本文描述 v1.6 时的行为。之后计费口径扩展为余额 / 订阅 / 云账单三态互斥（v1.7），设置页拆为原生 / 插件 / 提醒三组（v1.19），并加入插件自更新（v1.19）与单 profile 探测（见 `docs/CODE-AUDIT.md` P0-3）。以当前 README 与 `src/constants.js` 为准，不要按本文实现。
