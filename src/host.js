...
// 令牌的绑定/续期/写回由独立插件 dsh-chatgpt-sub 维护，本插件不写回、不续期、不注入凭据。
...
import { FIELD_REGISTRY, PRESET_COLOR_NAMES, PROVIDER_DEFAULT_CURRENCY, PROVIDER_IDENTITY } from './constants.js'
// 版本身份与语义化比较的唯一来源（包名/registry 地址/包版本/semver 比较，见 src/version.js）
import { PACKAGE_DIR, PACKAGE_FILE, UPDATE_REGISTRY_URL, compareSemver, packageVersion } from './version.js'
...
// PACKAGE_FILE/DIR：自更新的目标边界（写文件一律经 resolveSafePath 限定在它之内），定义见 version.js
...
// stableVersion / compareVersions 的旧名已并入 version.js（parseSemver / compareSemver），
// tests/test-update-check.cjs 改为直接从 version.js 取行为，见该文件头部说明。
...
function isLoadedAsProfilePlugin() {
  try {
    ...
    if (process.platform === 'win32' || process.platform === 'darwin') {
      return real.toLowerCase().startsWith(profilesRoot.toLowerCase())
    }
    return real.startsWith(profilesRoot)
  } catch { return false }
}

// compareVersions 的旧名已并入 version.js 的 compareSemver，下两处调用已同步改名。

// 运行中的 profile 名。DSH CLI 强制要求 --profile <name>，拿不到时（灰度桌面端是
// Electron 启动，参数里没有 --profile）先看本机实际有几个 profile：恰好只有一个时用它
// （桌面端只有 desktop，网页端只有 web）；多个或读不到时退回 web（安装脚本默认值）。
// 平台分支只在这一处做一次：目录探测失败绝不抛错，只影响更新命令里 profile 名的准确度。
function runningProfileName() {
  ...
  try {
    const entries = readdirSync(join(dshHomeDir(), 'profiles'), { withFileTypes: true })
      .filter((entry) => entry.isDirectory() && entry.name.length > 0 && !entry.name.startsWith('.'))
      .map((entry) => entry.name)
    if (entries.length === 1) return entries[0]
  } catch { /* 读不到目录就按旧逻辑退回 web */ }
  return 'web'
}
...
// 三条映射只查 PROVIDER_IDENTITY（定义见 constants.js）：新增服务商加表行即可，
// 再也不用在三条链里各写一遍。minimax 两站共用同一账户记账、订阅源各自独立——差异在表里，不在代码里。
function accountForProvider(pid) {
  if (!pid) return null
  const meta = Object.hasOwn(PROVIDER_IDENTITY, pid) ? PROVIDER_IDENTITY[pid] : null
  return meta ? meta.account : null
}

// ---------- 余额来源映射（与 accountForProvider 是两条独立的轴，严禁合并） ----------
// accountForProvider 回答的是「这笔花费记在哪个账户名下」，而花费索引是**增量落盘**的：
// 给一张老记录已经用过的 provider 新增账户键，历史记录不会跟着搬家（它们留在「无主」桶里），
// 同时账户过滤被打开 —— 用户看到的是「本会话花费突然归零」。
// 余额来源只回答「从哪里读余额」，与花费归属互不读写，因此单独一张表。
// 目前只有一项：DSH 桌面端内置的 DeepSeek 账号（无 API Key，余额走宿主账号服务）。
const PLATFORM_BALANCE_PROVIDERS = ['deepseek-account']
function balanceAccountForProvider(pid) {
  if (pid && PLATFORM_BALANCE_PROVIDERS.indexOf(pid) >= 0) return pid
  return accountForProvider(pid)
}
...
function subscriptionSourceFor(providerId) {
  const meta = providerId && Object.hasOwn(PROVIDER_IDENTITY, providerId) ? PROVIDER_IDENTITY[providerId] : null
  return meta ? meta.subscription : null
}

// 云账单 provider → 账单源标识（together / fireworks / amazon-bedrock / cloudflare）；非账单型 → null（FR-14）
function billingSourceFor(providerId) {
  const meta = providerId && Object.hasOwn(PROVIDER_IDENTITY, providerId) ? PROVIDER_IDENTITY[providerId] : null
  return meta ? meta.billing : null
}
...
// 与 parseFiniteNonNegativeAmount 同口径，名字保留（测试按契约引用），实现只调一份。
function minimaxNumericField(value) {
  return parseFiniteNonNegativeAmount(value)
}
...
// 快照合并（订阅/账单共用）：失败保留旧 data/fetchedAt、只换 error；成功替换 data 并刷新时间。
// fallbackCode 区分两条通道的兜底错误（旧快照与新错误都没有时的最后一道，平时走不到）。
function mergeSnapshotResult(prev, result, translate, fallbackCode) {
  if (translate === undefined) translate = t
  if (!result || result.error) {
    return {
      data: prev && prev.data ? prev.data : null,
      fetchedAt: prev && prev.fetchedAt ? prev.fetchedAt : null,
      error: result ? result.error : { kind: 'exception', code: fallbackCode, message: translate('error.' + fallbackCode) },
    }
  }
  return { data: result.data || null, fetchedAt: Date.now(), error: null }
}
// 对外名字保留（测试按名抽取快照语义、check-host 按名断言存在）：行为与原来逐字一致。
function mergeSubscriptionResult(prev, result, translate) {
  return mergeSnapshotResult(prev, result, translate, 'subscription.request-failed')
}
// 快照引擎（订阅/账单共用）：并发去重 + seq 防旧覆盖 + 失败退避 + 新鲜度判定。
// 数据投影（out 对象形状）两边完全不同，留在各自 RPC 里手写；这里只收机械部分。
// merge(snap, result, translate) 由调用方注入；fetch 统一按 src.fetch(arg) 调用，
// 不需要路由参数的源忽略 arg（与原来 kickBillingRefresh 不传参等价）。
function createSnapshotEngine(sources, options) {
  const snapshots = Object.create(null)
  const seq = Object.create(null)
  const inFlight = Object.create(null)
  const requested = Object.create(null)
  const lastFailAt = Object.create(null)
  const merge = options.merge
  const normalize = options.normalize || null
  const translate = options.translate
  function kick(key, arg) {
    const src = sources[key]
    if (!src) return Promise.resolve()
    if (inFlight[key]) return inFlight[key]
    const n = (seq[key] || 0) + 1
    seq[key] = n
    inFlight[key] = src.fetch(arg).then(function (result) {
      if (seq[key] === n) {
        if (normalize && result && result.data) result.data = normalize(result.data)
        snapshots[key] = merge(snapshots[key], result, translate)
        lastFailAt[key] = result && result.error ? Date.now() : 0
      }
    }).catch(function (err) {
      if (seq[key] === n) {
        snapshots[key] = merge(snapshots[key], { error: { kind: 'exception', message: String((err && err.message) || err) } }, translate)
        lastFailAt[key] = Date.now()
      }
    }).finally(function () {
      inFlight[key] = null
    })
  ...
  return {
    kick: kick,
    markRequested: function (key) { requested[key] = true },
    get: function (key) { return snapshots[key] || { data: null, fetchedAt: null, error: null } },
    lastFailAt: function (key) { return lastFailAt[key] || 0 },
    isStale: function (key, refreshMs, backoffMs, nowMs) {
      const snap = snapshots[key] || { fetchedAt: null }
      return (!snap.fetchedAt || (nowMs - snap.fetchedAt) > refreshMs) && (nowMs - (lastFailAt[key] || 0)) >= backoffMs
    },
    refreshActive: function (refreshMs, backoffMs, argForKey) {
      const nowMs = Date.now()
      for (const key in sources) {
        if (!requested[key]) continue
        if (nowMs - (lastFailAt[key] || 0) < backoffMs) continue
        kick(key, argForKey ? argForKey(key) : undefined)
      }
    },
  }
}
...
// ---------- DSH 桌面端内置账号余额解析（宿主 deepseekAccount 服务） ----------
// 平台把金额作为**字符串**下发（Web 端交给 big.js，其十进制语法允许省略整数/小数部分与十进制
// 指数，例如 '0E-16'、'5.0000000000000000'）。这里沿用 parseFiniteNonNegativeAmount 的严格口径：
// 形态不认识就让整条快照失败，由调用方保留上一次好数据 —— 宁可继续显示旧数，也不显示一个
// 好看但造出来的 0。
function platformWalletSum(wallets, currency) {
  let amount = 0
  let seen = false
  for (const wallet of wallets) {
    if (!wallet || wallet.currency !== currency) continue
    const parsed = parseFiniteNonNegativeAmount(wallet.balance)
    if (parsed == null) return { ok: false, seen: true, amount: 0 }
    amount += parsed
    seen = true
  }
  return { ok: true, seen: seen, amount: amount }
}

// 官方 total_balance = granted + topped_up，所以充值钱包（value）加赠送钱包（bonusWallets）求和：
// 「内置账号」与「API Key」两条路读到的是同一个数，用户换通道不会被两个余额吓到。
// 币种优先 CNY、其次 USD（与 DeepSeek API Key 路同一先例）；两个币种都没有钱包 → null。
function parsePlatformAccountBalance(result) {
  if (!result || result.status !== 'ready') return null
  const toppedUpWallets = Array.isArray(result.value) ? result.value : []
  const grantedWallets = Array.isArray(result.bonusWallets) ? result.bonusWallets : []
  for (const currency of ['CNY', 'USD']) {
    const toppedUp = platformWalletSum(toppedUpWallets, currency)
    const granted = platformWalletSum(grantedWallets, currency)
    if (!toppedUp.ok || !granted.ok) return null
    if (!toppedUp.seen && !granted.seen) continue
    return {
      currency: currency,
      total: toppedUp.amount + granted.amount,
      granted: granted.amount,
      toppedUp: toppedUp.amount,
    }
  }
  return null
}
...
// 注意与 parseFiniteNonNegativeAmount 的区别：limit 语义允许负数哨兵值，这里只要求有限，
// 不另做 >= 0 截断——两处不可合并，合了会把上游的负数语义吞掉。
...
// dsh-chatgpt-sub 负责；本插件只保留 readCodexAuthFile（只读令牌用于额度请求） ----------
...
// 校验 → 归一化 → 去重 → 收集，一处实现（三处调用：快照+流水加载、冷归档读取、导出聚合）。
// 去重键由 usageRecordKey 派生，调用方只给 (record, index, source)，source 区分数据来源。
function createRecordCollector() {
  const records = []
  const seen = new Set()
  return {
    records: records,
    add: function (record, index, source) {
      if (!isValidUsageRecord(record)) return false
...
function loadUsageRecords() {
  ...
  function add(record, index, source) {
    ...
    collector.add(record, index, source)
  }
  ...
  try {
    ...
    for (const name of files) {
      ...
      raw.split('\n').forEach(function (line, index) {
        if (!line.trim()) return
        try {
          collector.add(JSON.parse(line), index, 'archive:' + name)
...
function sanitizeSettings(raw) {
  ...
  // 旧设置没有 infoDensity 时沿用旧版完整模式，避免一次升级突然收起用户正在看的内容。
...
// 写满 + fsync 的底层原语（短写抛调用方给定的消息，其余错误原样抛）。
// 上层分两个耐久档：atomicReplaceFile（tmp + rename，必须不能丢）与直接调用（journal 追加的
// O_APPEND 语义不能经 tmp；自更新状态/日志是 best-effort，走自己的 try/catch，不合进来）。
function writeFullySync(filePath, content, flags) {
  let fd = null
  try {
    ...
    while (offset < bytes.length) {
      ...
      if (!written) {
        const err = new Error('short write to ' + filePath)
        err.code = 'BIB_SHORT_WRITE'
        throw err
      }
      offset += written
    }
...
// 原子替换写：tmp 写满 + fsync 后 rename 覆盖（避免半截文件），可选先把旧文件转成备份。
function atomicReplaceFile(filePath, content, options) {
  const opts = options || {}
  if (opts.ensureDir) mkdirSync(opts.ensureDir, { recursive: true, mode: 0o700 })
  const tmp = (opts.tmpPrefix || filePath + '.tmp.') + process.pid + '.' + randomUUID()
  writeFullySync(tmp, content, 'w')
  if (opts.backupPath && existsSync(filePath)) {
    try { renameSync(filePath, opts.backupPath) } catch (err) { /* 下面的替换仍是原子操作 */ }
  }
  renameSync(tmp, filePath)
}
// settings 落盘：目录确保 + 短写时翻译报错（open/rename 失败沿用原文，调用方按 code 归一）。
function writeFileAtomic(filePath, content, translate) {
  if (translate === undefined) translate = t
  mkdirSync(DATA_DIR, { recursive: true, mode: 0o700 })
  try {
    atomicReplaceFile(filePath, content, {})
  } catch (err) {
    if (err && err.code === 'BIB_SHORT_WRITE') throw new Error(translate('host.settingsFileCouldNotBe'))
    throw err
  }
}
  ...
  apply(ctx) {
    ...
    function selfUpdateState() {
      ...
      return {
        ...
        runningVersion: current, diskVersion: current, updated: false, disabled: true, lastErrorKind: null,
      }
    }
    // ---------- 更新状态的唯一出口（2026-09-26）----------
    // 所有「版本与更新」相关 RPC 都返回这一份 payload，形状完全一致。
    // 为什么必须只有一个出口：客户端以前拿到动作结果后会把它与轮询到的状态「拼」在一起
    // （Object.assign(prev, res)），而动作结果少几个字段 —— 于是点完检查更新，结论行还会说
    // 「已是最新」，要等最多 15 秒的下一次轮询才对上（界面在说谎）。
    // 现在客户端只认这一个形状，动作完成后重新读一次即可，不存在半状态。
    async function updateStatePayload() {
      ...
      const delta = compareSemver(current, state.runningVersion)
      return Object.assign({}, state, {
        latest: latest,
        diskVersion: current,
        available: !!latest && compareSemver(latest, current) > 0,
    ...
    function modelCurrency(provider, model) {
      ...
      if (Object.hasOwn(PROVIDER_DEFAULT_CURRENCY, provider)) return PROVIDER_DEFAULT_CURRENCY[provider];
    ...
    // ---------- 桌面端内置 DeepSeek 账号（deepseek-account）的余额来源 ----------
    // 这一路没有 API Key、也没有 HTTP：余额由宿主账号服务 deepseekAccount 直接给出 ——
    // 登录凭据由宿主持有，插件永远看不到，也就不存在泄漏的可能。
    // 读取一律走 ctx.get('deepseekAccount')：cordis 4 裸读未在 inject 里声明的服务属性会抛
    // `cannot get property "X" without inject`，而且该服务在部分宿主上本就不存在。
    function accountService() {
      if (typeof ctx.get !== 'function') return null;
      try { return ctx.get('deepseekAccount'); } catch { return null; }
    }

    // AccountClientMetadata：宿主用这三个字段拼 x-client-version / x-client-locale /
    // x-client-timezone-offset。locale 取自翻译器（宿主语言的唯一判定入口）；
    // 时区按平台约定是「东为正的秒数」，正好是 getTimezoneOffset() 的相反数。
    // version 填本插件版本：DSH 自己的版本留在 Electron 主进程里，不作为服务暴露给插件，
    // 插件侧拿不到；填自己的语义化版本号，至少保证请求头里始终是一个真实可读的值。
    function accountClientMetadata() {
      return {
        version: packageVersion(),
        locale: t.locale() === 'en' ? 'en-US' : 'zh-CN',
        timezoneOffsetSeconds: -new Date().getTimezoneOffset() * 60,
      };
    }

    // 返回 { data } 或 { error }（与本文件其它余额分支同型）。四种降级都给明确原因，绝不抛：
    // 服务缺席（老宿主）/ 未登录 / 取数失败 / 金额形态不认识。
    async function fetchDeepSeekAccountBalance() {
      const account = accountService();
      if (!account || typeof account.getBalance !== 'function') {
        return { error: { kind: 'credentials', code: 'balance.account-unavailable', message: t('error.balance.account-unavailable') } };
      }
      try {
        // 未登录、以及 401/凭据过期，宿主都收敛成 null（不是抛错）
        const result = await account.getBalance(accountClientMetadata());
        if (result == null) {
          return { error: { kind: 'no-key', code: 'balance.account-signed-out', message: t('error.balance.account-signed-out') } };
        }
        if (result.status !== 'ready') {
          // status: 'failed' —— 网络或协议失败，宿主已把原因归一到这一种形态
          return { error: { kind: 'http', code: 'balance.account-request-failed', message: t('error.balance.account-request-failed') } };
        }
        const parsed = parsePlatformAccountBalance(result);
    ...
    // balance_infos 列表解析（DeepSeek / Kimi 同构；granted/toppedUp 缺失按 0 计）。
    // 币种优先级必须由调用方声明：Kimi 国际站与中国站用同一 API 形状，却不是同一结算域。
    function parseBalanceInfos(body, currencyOrder) {
      // Kimi 的公开余额 API 已出现过两种 envelope：旧版 balance_infos[] 与新版
      // data.{available_balance,cash_balance,voucher_balance}。只认旧版会在服务端平滑升级后
      // 把真实余额误判成解析失败；同时不把任意数字字段当余额，避免误读额度/账单金额。
      const payload = body && body.data && typeof body.data === 'object' && !Array.isArray(body.data) ? body.data : body;
      const list = payload && Array.isArray(payload.balance_infos) ? payload.balance_infos : [];
      let rec = null;
      const preferred = Array.isArray(currencyOrder) && currencyOrder.length > 0 ? currencyOrder : ['CNY', 'USD'];
      for (const currency of preferred) {
        for (let i = 0; i < list.length; i++) {
          if (list[i] && list[i].currency === currency && parseFiniteNonNegativeAmount(list[i].total_balance) != null) { rec = list[i]; break; }
        }
        if (rec) break;
      }
      const total = rec ? parseFiniteNonNegativeAmount(rec.total_balance) : null;
      if (rec && total != null) {
        ...
        return { currency: rec.currency, total: total, granted: granted == null ? 0 : granted, toppedUp: toppedUp == null ? 0 : toppedUp };
      }
      const available = parseFiniteNonNegativeAmount(payload && payload.available_balance);
      const cash = parseFiniteNonNegativeAmount(payload && payload.cash_balance);
      const voucher = parseFiniteNonNegativeAmount(payload && payload.voucher_balance);
      const fallbackTotal = cash != null && voucher != null ? cash + voucher : null;
      const currentTotal = available != null ? available : fallbackTotal;
      if (currentTotal == null) return null;
      return {
        currency: payload.currency === 'USD' || payload.currency === 'CNY' ? payload.currency : preferred[0],
        total: currentTotal,
        granted: voucher == null ? 0 : voucher,
        toppedUp: cash == null ? 0 : cash,
      };
    }
    ...
    const PROVIDERS = {
      deepseek: {
        ...
        estimate: false,
        parseBalance: parseBalanceInfos,
      },
      // DSH 桌面端内置 DeepSeek 账号：与上面 API Key 路是同一品牌的**两条通道**，账户键刻意分开 ——
      // 两条路的凭据与失效原因完全不同（「没配 API Key」和「账号没登录」是两句不同的话），
      // 混成一个键会让一种失效掩盖另一种。余额来自宿主账号服务，不走凭据、不发 HTTP。
      'deepseek-account': {
        id: 'deepseek-account', displayName: 'DeepSeek', credential: null, currency: 'CNY',
        balanceAPI: null,
        estimate: false,
        platformBalance: function () { return fetchDeepSeekAccountBalance(); },
      },
      openai: {
      ...
      moonshotai: {
        id: 'moonshotai', displayName: 'Moonshot AI', credential: 'MOONSHOT_API_KEY',
        balanceAPI: 'https://api.moonshot.ai/v1/users/me/balance',
        estimate: false,
        parseBalance: function (body) { return parseBalanceInfos(body, ['USD', 'CNY']); },
      },
      'moonshotai-cn': {
        id: 'moonshotai-cn', displayName: 'Moonshot AI CN', credential: 'MOONSHOT_API_KEY',
        balanceAPI: 'https://api.moonshot.cn/v1/users/me/balance',
        estimate: false,
        parseBalance: function (body) { return parseBalanceInfos(body, ['CNY', 'USD']); },
      },
    ...
    // 取余额有且只有三种互斥方式，判定只写在这一处（refreshProviderBalance 只负责分发）：
    //   platform  宿主账号服务：既没有凭据也没有 HTTP（桌面端内置账号）
    //   estimate  记账回退：估算余额 = 起始充值额 - 累计花费（无公开余额接口的服务商）
    //   http      凭据 + HTTP：本文件里最常规的一路
    function balanceSourceKind(prov) {
      if (typeof prov.platformBalance === 'function') return 'platform';
      if (!prov.balanceAPI) return 'estimate';
      return 'http';
    }
    ...
    function refreshProviderBalance(pid, force) {
      ...
      balanceSeq[pid] = seq;
      const source = balanceSourceKind(prov);
      if (source === 'estimate') {
      ...
      const task = (async function () {
        if (source === 'platform') {
          // 平台账号路（见 fetchDeepSeekAccountBalance）：失败一律保留旧 data/fetchedAt，只换 error。
          let result = null;
          try {
            result = await prov.platformBalance();
          } catch (err) {
            result = { error: { kind: 'exception', message: String((err && err.message) || err) } };
          }
          if (balanceSeq[pid] !== seq) return
          if (result && result.error) {
            balances[pid] = { data: balances[pid] && balances[pid].data, fetchedAt: balances[pid] && balances[pid].fetchedAt, error: result.error };
            return;
          }
          balances[pid] = { data: (result && result.data) || null, fetchedAt: Date.now(), error: null };
          return;
        }
        let cred = null;
        try {
    ...
    const subscriptionSourceProvider = Object.create(null); // { [sourceKey]: providerId }——记住请求该源时的 provider（zai 系按 provider 路由 host/凭据），供引擎路由参数用
    // 快照状态（snapshots/seq/inFlight/requested/lastFailAt）收进下面的两个引擎实例，不再各写一套。

    // FR-8 / D7：Codex / ChatGPT 订阅卡（纯本地通道）
    // 只读令牌：令牌由独立插件 dsh-chatgpt-sub 维护，本插件不续期、不写回、不注入凭据；
    ...
    async function resolveCommandCodeKey() {
      const configured = await resolveWithFallback(['COMMAND_CODE_API_KEY', 'CMD_API_KEY'])
    ...
    async function resolveMinimaxKey(providerId) {
      ...
      return resolveWithFallback([primary, fallback])
    }
    ...
    // 按优先级逐个试凭据名，首个命中即返回（无副作用的纯查找，顺序即优先级）。
    async function resolveWithFallback(names) {
      for (const name of names) {
        const value = await resolveCredentialValue(name);
    ...
    async function resolveXiaomiRegionKey(region) {
      ...
      return resolveWithFallback(primary ? [primary, 'XIAOMI_API_KEY'] : ['XIAOMI_API_KEY']);
    }
    ...
    async function getSubscriptionSnapshotRpc(selection, force) {
      ...
      subscriptionEngine.markRequested(sourceKey); // 该源进入 60s 周期刷新
      subscriptionSourceProvider[sourceKey] = sel.provider; // 记住 provider（zai 系路由需要）
      const snap = subscriptionEngine.get(sourceKey);
      const nowMs = Date.now();
      ...
      if (force || subscriptionEngine.isStale(sourceKey, SUBSCRIPTION_REFRESH_MS, SUBSCRIPTION_RETRY_BACKOFF_MS, nowMs)) {
        const inflight = subscriptionEngine.kick(sourceKey, subscriptionSourceProvider[sourceKey]);
        // 从未成功过（无旧数据）或强制刷新 → 等本次结果返回最新数据（含错误）；
        // 已有旧数据的普通刷新 → 后台刷新，本次直接返回快照（不阻塞轮询）
        if (!snap.data || force) await inflight;
      }
      const cur = subscriptionEngine.get(sourceKey);
      if (cur.data) {
    ...
    // 机械部分（并发去重 / seq 防旧覆盖 / 失败退避 / 新鲜度判定）只在 createSnapshotEngine 里写一遍。
    const subscriptionEngine = createSnapshotEngine(SUBSCRIPTION_SOURCES, { merge: mergeSubscriptionResult, translate: t });
    const billingEngine = createSnapshotEngine(BILLING_SOURCES, {
      merge: function (prev, result) { return mergeSnapshotResult(prev, result, t, 'billing.request-failed'); },
      // FR-14：适配器原始输出统一经 normalizeAccountStatus 收敛到客户端契约
      normalize: function (data) { return normalizeAccountStatus(data.kind, data); },
    });
    // 60s 周期刷新：仅刷新客户端请求过的源（余额制模式下不打扰未公开的订阅接口）；失败退避期内跳过
    function refreshActiveSubscriptions() {
      subscriptionEngine.refreshActive(SUBSCRIPTION_REFRESH_MS, SUBSCRIPTION_RETRY_BACKOFF_MS, function (key) { return subscriptionSourceProvider[key]; });
    }
    function refreshActiveBilling() {
      billingEngine.refreshActive(BILLING_REFRESH_MS, BILLING_RETRY_BACKOFF_MS);
    }
    ...
    async function getBillingSnapshotRpc(selection, force) {
      ...
      if (!key) return out;
      out.type = key;
      billingEngine.markRequested(key); // 该源进入 60s 周期刷新
      const snap = billingEngine.get(key);
      const nowMs = Date.now();
      // force（客户端打开/刷新网页后的首启窗口）绕过新鲜度与失败退避，当场重查最新账单
      if (force || billingEngine.isStale(key, BILLING_REFRESH_MS, BILLING_RETRY_BACKOFF_MS, nowMs)) {
        const inflight = billingEngine.kick(key);
        if (!snap.data || force) await inflight;
      }
      const cur = billingEngine.get(key);
      out.data = cur.data;
      out.fetchedAt = cur.fetchedAt;
      out.error = cur.error;
      return out;
    ...
    function nextSwitchAt(nowMs) {
      ...
      for (let day = 0; day <= 8; day++) {
        for (let i = 0; i < bounds.length; i++) {
          const at = new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate() + day, bounds[i], 0, 0) - BEIJING_OFFSET_MS).getTime();
    ...
    function nextPeriodLabel(nowMs) {
      ...
      const d = new Date(at + BEIJING_OFFSET_MS);
    ...
    function activeBalanceSummary(nowMs, selection) {
      ...
      // 没有身份映射才是真的未知服务商。已知的 DSH 预置服务商即使没有公开账户
      // 接口，也仍应保留模型识别与本地 token 账本，不能误报为“不支持”。
      if (pid === null) {
      ...
      if (!prov) {
        return {
          ...
          displayName: providerDisplayFromCache(activeProvider, providerNameCache, PROVIDER_DISPLAY, t),
          selectionPending: false,
          unmapped: false,
          accountDataUnavailable: true,
          data: null,
          fetchedAt: null,
          error: null,
          alert: null,
    ...
    function allUsageRecordsForExport() {
      ...
      const collector = createRecordCollector();
      const records = collector.records;
      usageRecords.forEach(function (record, index) { collector.add(record, index, 'current') });
      archived.records.forEach(function (record, index) { collector.add(record, index, 'archive-export') });
      return {
        records: records.sort(function (a, b) { return a.ts - b.ts }),
    ...
    function writeSummariesFile(foldedUpTo, sessionsDelta, accountTotals) {
      ...
      atomicReplaceFile(USAGE_SUMMARIES_FILE, JSON.stringify(payload), { tmpPrefix: USAGE_SUMMARIES_TEMP_PREFIX, backupPath: USAGE_SUMMARIES_BACKUP_FILE });
    }
    ...
    function maybeCompactJournal() {
      try {
        ...
        atomicReplaceFile(USAGE_JOURNAL_FILE, kept.length > 0 ? kept.join('\n') + '\n' : '', { tmpPrefix: USAGE_JOURNAL_FILE + '.compact.' });
    ...
    // 账本域的写满 + fsync：短写翻译报错，其余原样抛（与 settings 的 writeFileAtomic 同策略）。
    function writeAndSync(filePath, content, flags) {
      try {
        writeFullySync(filePath, content, flags);
      } catch (err) {
        if (err && err.code === 'BIB_SHORT_WRITE') throw new Error(t('host.spendLedgerCouldNotBe'));
        throw err;
      }
    }
    ...
    function flushSave() {
      ...
      try {
        ...
        atomicReplaceFile(DATA_FILE, JSON.stringify(usageRecords), { tmpPrefix: DATA_TEMP_PREFIX, backupPath: DATA_BACKUP_FILE, ensureDir: DATA_DIR });
    ...
    const ROUTES = {
      getUpdateInfo: async function () {
        // 客户端每 60 秒读一次这个接口，所以本地副本更新完后提醒会自己消失，不必刷新页面或重启宿主。
        // 摘要取自同一个 payload，避免出现第二份版本比较逻辑。
        // 命令按安装形态区分，避免把 link: 安装的用户引导到 npm 命令而丢掉本地代码。
        const payload = await updateStatePayload()
        return Object.assign({}, updateCommandForInstall(), {
          current: payload.diskVersion,
          ...
          diskVersion: payload.diskVersion,
          pendingRestart: payload.pendingRestart,
          autoUpdate: payload.autoUpdate,
          holdVersion: payload.holdVersion || null,
      ...
      // 检查更新（只读）：只问 npm「有没有更高的版本」，然后回报。
      // 绝不下载、绝不替换包文件 —— 这是「检查更新」按钮唯一该做的事。
      // 走引擎而不是读 host 的 TTL 缓存：用户点了按钮就必须真去问一次（缓存命中时那次
      // 查询可能要等 15 分钟才过期，等于按钮什么都没干）。
      checkUpdate: async function () {
        if (selfUpdater) {
          const state = await selfUpdater.run({ mode: 'check', manual: true })
          // 顺手把这次问到的版本灌进缓存，让信息栏与后续状态读取立刻与检查结果一致。
      ...
      // 安装（唯一的写动作）：由用户在手动方式下点「更新到 X」、或点「允许更新到 X」触发。
      // 全自动模式不需要它 —— 那条路由启动流程自己走 install。
      // force=true 表示「我明确要装这个版本」，用于解除回滚后的暂缓（holdVersion）。
      installUpdate: async function (args) {
        if (selfUpdater) {
          const force = !!(args && typeof args === 'object' && args.force === true)
          const state = await selfUpdater.run({ mode: 'install', manual: true, force: force })
          if (state && typeof state.latest === 'string') {
            updateLatestCache.value = state.latest
            updateLatestCache.expiresAt = Date.now() + updateLatestTtlMs()
          }
      ...
      // 旧客户端页面（升级后还没重启、WebView 里仍是上一版界面）调用的老名字。
      // 老名字的语义就是「检查并安装」，因此这里保持安装，不改语义 ——
      // 改掉会让旧页面点「检查更新」直接报错，那是更糟的体验。
      runUpdateCheck: async function (args) {
        return ROUTES.installUpdate(args)
      },
      // 更新方式（全自动 / 手动）。手动方式下自动流程只查不装，必须由用户点「更新到 X」。
      setUpdateAuto: async function (args) {
        if (selfUpdater) {
          const enabled = !(args && args.enabled === false)
          selfUpdater.setAutoUpdate(enabled)
        }
        return updateStatePayload()
      },
      // 回滚到最近一次更新的备份（新版启动异常时的人工兜底）。
      // 结果里额外带回 rollback 结论：失败（没有备份 / 没有可回滚版本）必须让界面说出来，
      // 不能点一下什么都没发生。
      rollbackUpdate: async function () {
        const result = selfUpdater ? selfUpdater.rollback() : { restored: false, reason: 'disabled' }
        return Object.assign({}, await updateStatePayload(), { rollback: result })
      },
      ...
      setFieldConfig: function (args) {
        ...
        if (hasInfoDensityPatch) {
          if (fieldSettings.infoDensity !== normalizedInfoDensity) {
            fieldSettings.infoDensity = normalizedInfoDensity
            config.infoDensity = normalizedInfoDensity
            changed = true;
          }
        }
        if (changed) {
