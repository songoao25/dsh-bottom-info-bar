...
import { FIELD_REGISTRY, PRESET_COLOR_NAMES, PROVIDER_IDENTITY } from './constants.js'
...
export default {
  ...
  apply(ctx) {
    ...
    // balance_infos 列表解析（DeepSeek / Kimi 同构：优先可用的 CNY，再退 USD；
    // granted/toppedUp 缺失按 0 计）。两家 API 返回同一 envelope，各写一遍只会让修解析时漏改一家。
