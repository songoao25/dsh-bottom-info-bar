...
- 修法（官方布局，与姊妹插件 dsh-chatgpt-subscription 对齐）：包从 `plugin/` 移到**仓库根**（package.json / cordis.patch.yml / src / scripts / locale）；`lib/` **入库**（git 安装无需构建）；`prepack` → `prepublishOnly`（pnpm 只对 git 依赖跑 prepare/prepack，去掉后 git 安装不再弹「待批准的构建脚本」，发 npm 时仍重建）；发布链同步：release-please 包路径 `plugin` → `.` + manifest 键 + publish-npm + CI + dependabot；CI 加「Verify generated bundle is committed」。
