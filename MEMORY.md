...
- **严禁任何 Agent 自建或使用专属物。** 禁的不是某一种文件，而是「专属」本身：记忆文件/目录（如 `.workbuddy/memory/`、`.cursor/memory/`、任何未在本文件登记的记忆位置）、配置与指令文件（如 `CLAUDE.md`、`.cursorrules`、各类工具私有目录）一律不允许新建。本项目只用通用 Agent 资产：`MEMORY.md`、`AGENTS.md`、`docs/`、`scripts/`、`tests/`、`locale/`、`install.sh`/`install.ps1`。
- **记忆属于项目，不属于工具。** 换一个 Agent、换一个 IDE 接手时，必须能读到完全相同的上下文；写进工具私有目录等于把项目知识锁死在某一个工具里，下一个 Agent 看不到，等于丢失。
- 如果发现历史遗留的工具专属物，**迁移进通用位置后删除**（记忆进本文件，流程进 `docs/WORKFLOW.md`，硬约束进 `tests/test-source-guards.mjs`），不要两份并存（两份必然分叉）。
...
## 2026-09-26（用户拍板：禁令从记忆推广到一切 Agent 专属物）

- 两份工具专属记忆均已删：09-04 那份随 #70 删并迁移（`git log -- .workbuddy` 留档，提交 fc3bf9f）；09-26 那份未入库，本次审计迁入后删除（`git ls-files` 已干净，历史无残留）。
- 用户举一反三：本项目只能用通用 Agent 的东西。落地三处 —— ① 本文件使用规则改禁一切专属物并列出通用资产清单；② `AGENTS.md` 同步；③ 守卫 2 从 8 个记忆路径扩大到记忆/配置/指令全名单（20 目录 + 10 文件），CI 强制执行；`.gitignore` 同步加名单防误 add。
- 未动：主目录全局工具环境（`~/.workbuddy/MEMORY.md` 是用户指定的跨项目偏好存放处、`~/.codex/auth.json` 是本插件只读的订阅登录态）不属于本项目；仓库内其余引用残留全是禁令条文与守卫名单本身。
...
### 从用户决定出发重做设置页（feat，PR #154）

- 重新定义信息栏的最低承诺：服务商/模型、一项关键账户信息，以及真正需要处理的错误，不再能被逐条开关误关；完整模式只保留两个有意义的选择——是否显示账户详情、是否显示对话统计。旧的逐条显示与颜色设置仍保留在默认收起的「高级设置」，旧配置不会被升级覆盖。
- 新安装默认使用简洁模式；已有 settings.json 缺少密度字段时保持原来的完整模式，避免升级后突然改变用户正在看的内容。简洁模式仍只显示身份和一个关键余额/额度/账单数据，完整模式才显示重置时间、订阅到期、会话花费、预算、Token、缓存和上下文等细节。
- 新版本不再占用聊天信息栏，也不再提供点击复制更新命令：桌面端没有可靠的覆盖安装事务，复制 Web/终端命令会误导用户。版本信息保留作诊断数据；真正自动更新应由 DSH 客户端实现安全的安装事务，不能靠插件卸载/重装冒充。
- 验证与发布：`node tests/run-all.mjs` 全绿、`git diff --check` 通过、`npm pack --dry-run` 通过；PR #154 CI/CodeQL 绿并合入，release PR #155 合并，GitHub Release `v1.18.0` 与 Publish NPM workflow 成功。npm registry 可能短暂延迟，须另行读回 latest 后才可称 npm 已同步。

---

## 2026-09-25（v1.17.0）

### 简洁模式收口与设置页文案整理（feat，PR #151）

- 简洁模式不再只是收起原生统计行：余额制只显示「服务商 · 模型｜余额」；订阅制只显示优先级最高的一个额度窗口（5 小时 → 周 → 月）；云账单制只显示本计费周期花费或用量。时间、自定义文字、峰谷状态、各种倒计时、会话花费、预算、免费额度、更新提醒和上下文圆环均只在完整模式出现。错误与未保存提醒仍保持可见，不能因极简吞掉需要处理的问题。
- 设置页新增持久化的「简洁 / 完整」分段选择，和直接点击信息栏的切换使用同一份 `infoDensity` 设置；简洁不会改写用户已选字段，切回完整即可恢复。插件信息默认展开，完整模式专属的原生统计改为按需展开。
- 术语收口：把订阅按量账户的「充值余额」改成「可用余额」，credits 改成「剩余额度」，设置页说明明确两种数据来源，避免同一类账户信息出现重复或难懂的名字。
- 验证与发布：`node tests/run-all.mjs` 全绿、`git diff --check` 通过；PR #151 CI/CodeQL 绿并合入，release PR #152 合并，GitHub Release `v1.17.0`、Publish NPM workflow 成功，`npm view dsh-bottom-info-bar version dist-tags --json` 已读回 `version/latest = 1.17.0`。
...
- 根因链（三层，少看一层就会误判成「我们的包坏了」）：① DSH 0.1.7 的安装引导**明确让用户填 GitHub 仓库地址**（`installGuideGitHint`「插件在 GitHub 上的开源仓库地址」，示例就是 `https://github.com/author/dsh-plugin`）；② pnpm 对**根目录没有 package.json** 的仓库**不报错**，而是合成占位清单——实测装出来的是 `{"_pnpmPlaceholder":"This file was generated by pnpm. The original package did not contain a package.json."}`；③ DSH 装完读该清单，找不到 `dsh.bundle` → `ManagementFailure("not-bundle")` → 前端文案 `reasonNotBundle`。**文案把责任指向包，实际是地址指向了非包目录。**
- 修法（官方布局，与姊妹插件 dsh-chatgpt-sub 对齐）：包从 `plugin/` 移到**仓库根**（package.json / cordis.patch.yml / src / scripts / locale）；`lib/` **入库**（git 安装无需构建）；`prepack` → `prepublishOnly`（pnpm 只对 git 依赖跑 prepare/prepack，去掉后 git 安装不再弹「待批准的构建脚本」，发 npm 时仍重建）；发布链同步：release-please 包路径 `plugin` → `.` + manifest 键 + publish-npm + CI + dependabot；CI 加「Verify generated bundle is committed」。
- 顺带：host 的更新命令对**非注册表来源**（`github:` / `git+` / `git@` / http(s)）改为**重跑同一条 add**（npm 那条会被「已安装」挡下）；含 `#` 的地址会被 `shellQuote` 引号包住（否则 shell 当注释）。
- 验证（隔离 `DSH_HOME`，不碰本机 3080）：GitHub 地址（分支）安装 → 无构建授权提示、装上 1.16.0、bundle 自动选中、`--dump-config` 出现 `bottom-info-bar` 行；本地路径安装同样通过；重复执行同一条 add 可更新；全量 37 项测试全绿；构建字节级确定（连跑两次一致，重建后 `git status --porcelain -- lib` 为空）。
- **可复用经验 1（pnpm 占位清单）**：`_pnpmPlaceholder` 是「仓库根不是包」的唯一信号。看到用户报 `not-bundle`，先查仓库根有没有 package.json，不要先怀疑 `dsh.bundle` 写法。
- **可复用经验 2（rebase 会静默丢上游改动）**：把「整目录重命名」的提交 rebase 到同时改过这些文件的 main 上时，git 的 rename 检测**只对一部分文件生效**：本次 `src/host.js`、`src/client-bundle.js` 合并正常，而 `src/constants.js`、`src/locales.js`、`tests/check-host.*`、`tests/test-dual-mode.*` 等文件的上游改动被**静默丢弃**（无冲突、无任何提示，是测试报错才暴露）。**唯一可靠的核对方式**：对 `git diff --name-only <base>..origin/main` 里的每个文件，用 `diff <(git show origin/main:<旧路径>) <新路径>` 逐个比对，确认剩下的差异都是自己的有意改动。
- **可复用经验 3（type: module 会扩散到全仓库）**：包移到仓库根后，根的 `"type": "module"` 作用于整个仓库——原先靠「根目录没有 package.json」当 CJS 跑的 15 个 `.js` 测试全部 `require is not defined`，必须改 `.cjs`（Node 22+ 的语法探测只在没有显式 type 时生效）。
- 迁移：1.15.0 及更早用本地代码安装的用户，profile 里的 `link:` 指向 `<repo>/plugin`，该目录已不存在，需先 remove 再按新路径重装（已写进 README / INSTALL）。
...
- 顺带：同 profile 里 `dsh-chatgpt-sub` 等插件的描述仍只有中文（各自仓库的 package.json），要双语得在各自仓库加 `locale/*.json`。
...
- 真实 DSH UI 复核暂时被独立的 `dsh-chatgpt-sub` 语法错误（`missing ) after argument list`）阻断：服务在加载该插件时退出，非本插件错误；不得为此修改它。待其修复后，使用 `scripts/dshr` 重启并检查深色主题下的首屏、折叠和搜索交互，再走正常 `fix:` → PR → Release Please 发布链。
...
- **另一条待办**：`dsh-chatgpt-sub` 同步去掉 `.cgpt-page` 的 `max-width:760px`（同样的 P1），并把 `.cgpt-rowDesc` 对齐 `11.5/16`；已完成于 `c01cd4b`。
...
- 真实验证：重启 `dsh web` 后打开插件页，信息栏配置区块正常渲染（显示内容 / 账单数据 / 导出 / 清除 / 恢复默认），控制台零 `slot entry crashed`；同一轮也确认 `dsh-chatgpt-sub` 的插件页正常。
- 可复用经验：
