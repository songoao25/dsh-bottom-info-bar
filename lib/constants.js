...
//   2026-09-25 用户再次澄清组别的含义：组别就是「它属于谁」——**简洁模式只显示 plugin 组，
//   完整模式显示 native + plugin**。因此一个字段归哪一组，直接决定它在简洁模式下能不能出现。
//   上下文圆环虽然原型来自 DSH 原生底部栏，但已经被本插件接管、且始终渲染在主行（简洁模式可见的
//   那一行最右端），所以按 plugin 组对待 —— 否则它在简洁模式下会整块消失。
