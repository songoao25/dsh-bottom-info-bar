...
//
// 两个动作，不许合并（2026-09-26 用户报「点一下检查更新就直接装好了」后拆开）：
//   mode:'check'   → 只查版本，对磁盘纯读（不下载 / 不替换 / 不碰备份）；
//   mode:'install' → 唯一的写动作，只由「全自动模式的启动流程」或用户显式点「更新到 X」触发。
//（历史上只有一条路，检查按钮因此等于安装按钮 —— 这是本次拆分的全部理由。）
//
...
import { PACKAGE_NAME, UPDATE_REGISTRY_ORIGIN, compareSemver, parseSemver } from './version.js'
// 下游（测试）从本模块取版本 API：名字不变，转交 version.js（唯一的定义处）。
export { PACKAGE_NAME, compareSemver, parseSemver }
...
// 词法路径在包目录内还不够：若已有的中间目录是符号链接，`lib/x.js` 仍可能写到包外。
// 自更新只支持实体目录安装；遇到链接不猜其目标，直接拒绝整次更新。叶子文件本身是链接时
// rename 会替换链接而不会跟随它，因此只检查目录链即可。
function resolveSafePayloadTarget(baseDir, relative) {
  const full = resolveSafePath(baseDir, relative)
  if (!full || !isAllowedPayloadPath(relative)) return null
  const base = resolve(baseDir)
  try {
    if (lstatSync(base).isSymbolicLink()) return null
...
// 失败原因归一（2026-09-25 第三轮加入）：state.lastError 原来直接存技术报错（如
// "unexpected end of file"），界面只能把它原样甩给用户 —— 用户看到一行英文报错，既不懂也不知道
// 自己有没有风险。这里把报错映射成稳定代号，由界面翻译成人话；原始报错仍写进 update-log.jsonl
// 供排查（见 runOnce 的 catch）。
// 映射同时认「代号」与「原始报错」，因此历史 state 文件里存的老值也能被正确翻译。
// 顺序即优先级：更具体的模式必须排在更宽的模式前面，否则会被宽模式抢先命中
// （「download failed: too large」曾被 download-failed 抢走，2026-09-25 由新增的归一单测抓出）。
const UPDATE_ERROR_PATTERNS = [
  ['incomplete-download', /unexpected end of file|invalid tar|incorrect header check|invalid distance|invalid checksum|too short|not in gzip/i],
  ['integrity-mismatch', /integrity mismatch/i],
  ['too-large', /too large/i],
  ['download-failed', /download failed|empty body|fetch failed|abort/i],
  ['payload-mismatch', /package name mismatch|version mismatch|has no package\.json|not valid JSON|not semver/i],
  ['payload-unsafe', /path not allowed|path escapes|refusing to write outside/i],
  ['check-failed', /check-failed/i],
]
export const UPDATE_ERROR_KINDS = ['incomplete-download', 'integrity-mismatch', 'download-failed', 'too-large', 'payload-mismatch', 'payload-unsafe', 'check-failed', 'unknown']

export function describeUpdateError(value) {
  const text = typeof value === 'string' ? value : (value && value.message) || ''
  if (text.length === 0) return 'unknown'
  if (UPDATE_ERROR_KINDS.includes(text)) return text
  for (const [kind, pattern] of UPDATE_ERROR_PATTERNS) {
    if (pattern.test(text)) return kind
  }
  return 'unknown'
}

// parseSemver / compareSemver 见 version.js（唯一的定义处，本模块顶部转交）。
...
export function verifyPayload(files, options) {
  ...
  if (!Array.isArray(files)) throw new Error('payload files are invalid')
  const seenPaths = new Set()
  for (const file of files) {
    if (!file || typeof file.path !== 'string' || seenPaths.has(file.path)) {
      throw new Error('payload has duplicate or invalid path: ' + String(file && file.path))
    }
...
function listPayloadFiles(baseDir) {
  ...
  const walk = (dir, prefix) => {
    ...
    for (const name of names) {
      ...
      let info
      // 不跟随符号链接：备份阶段绝不能因为一个异常链接而读取包目录以外的文件。
...
export function applyPayload(files, context) {
  ...
  if (!Array.isArray(files)) throw new Error('applyPayload needs files')
  // 先完整预检，避免一个后置的异常路径导致前面的合法文件已经被替换。
  const targets = files.map((file) => {
    const path = file && file.path
    const target = resolveSafePayloadTarget(packageDir, path)
    if (!target) throw new Error('refusing to write outside package or through a symbolic link: ' + String(path))
    return { file, target }
  })
...
export function createSelfUpdater(options) {
  ...
  // 并发去重按动作分槽：检查是纯读、安装是写，两者语义不同，不能共用一个在途 promise ——
  // 共用会让「检查在跑时来的安装请求」变成什么都不做的空转（拿到的是检查的结果）。
  const inFlight = { check: null, install: null }
  ...
  async function runOnce(options) {
    ...
    // 2026-09-26：「看一眼」与「装上去」必须是两个动作。
    // 用户点「检查更新」只是想确认有没有新版，而本引擎从前的 runOnce 只有一条路 —— 检查、下载、
    // 替换一气做完，于是「检查更新」按钮等于「立即安装」（用户原话：「点了一下就直接装好了」）。
    // mode:'check' 在确认远端确实更高之后立刻返回：不下载、不替换、不碰备份、不记 lastError
    // （除非这次检查本身失败），因此它对磁盘是纯读的。
    const checkOnly = !!(options && options.mode === 'check')
    ...
    if (checkOnly) {
      // 到这里已经证明「远端确实比在跑的版本高」，检查的使命就结束了。
      // 上一次检查的失败记录在这里清掉（网络恢复了）；更新失败的记录必须留着 ——
      // 「上次更新失败」是界面上的逃生门依据，不能被一次顺利的检查顺手抹掉。
      if (state.lastError && describeUpdateError(state.lastError) === 'check-failed') state.lastError = null
      saveState()
      return Object.assign({}, state, { latest: latest.version, checkedOnly: true })
    }
    ...
    if (diskVersion() === latest.version) {
      // 磁盘已是新版：等用户重启，别重复下载。
      // 这里**不能**再挂 `&& !manual`：手动点「检查更新」只表示「用户在自动关掉时授权下载」，
      // 不是「绕过防重」—— 原来那样写会让手动方式下连点几次就重复下载、重复替换、多留几份备份
      // （2026-09-25 第三轮用户问「更新到底实现没有」时顺手查出来的）。
    ...
    try {
      ...
      // 状态里只存代号（界面据此翻译成人话），原始报错写进日志供排查。
      const raw = error && error.message ? error.message : 'update-failed'
      state.lastError = describeUpdateError(raw)
      saveState()
      appendLog({ event: 'update-failed', version: latest.version, kind: state.lastError, message: raw })
    }
    return Object.assign({}, state, { latest: latest.version })
  }
  ...
  return {
    ...
    getState() {
      return Object.assign({}, state, {
        ...
        // 失败原因额外给出代号：界面据此翻译成人话。历史 state 文件里存的是原始报错，
        // 由 describeUpdateError 现场归一，因此老数据同样能显示成人话。
        lastErrorKind: state.lastError ? describeUpdateError(state.lastError) : null,
      })
    },
    // 检查与安装共用一条流水线（runOnce 内部按 mode 分支），各自去重：
    //   run()               → 安装（全自动模式的启动流程；有新版就装）
    //   run({ mode: 'check' }) → 只检查（用户点「检查更新」；对磁盘纯读）
