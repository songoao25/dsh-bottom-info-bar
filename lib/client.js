...
const FIELD_REGISTRY = [
  ...
  // 插件字段 · 上下文占用圆环（DSH 原生 ContextMeter 的接管版）：
  // 外观、几何与交互面板与原生一致，但显隐/配色并入本插件字段体系；
  // 始终渲染在本插件信息栏的主行（即「简洁模式」可见的那一行）最右端，原生那一份由样式隐藏。
  // 归 plugin 组：主行两种模式都在，按 native 组处理会让它在简洁模式下消失。
  { id: 'contextUsage', label: "field.contextUsage.label", group: 'plugin', modes: ['common'], colorKind: 'meter', note: "field.contextUsage.note" },
