...
信息栏有两种显示方式，点击即可切换，区别只有一处：**完整**在主行之上保留 DSH 原生统计行，**简洁**把它收起。两种方式都显示已启用的全部插件信息与提醒——看哪个字段，只由该字段自己的开关决定。两种方式都跟随 DSH 的深色/浅色主题。

## 简洁模式

简洁模式只收起原生统计行，主行照常显示已启用的插件信息。四种真实状态：浅色与深色、余额制与订阅额度制。
...
完整模式显示各窗口剩余额度（5 小时 / 周 / 月）与距下次重置的倒计时——两者始终取自同一个窗口，不会互相矛盾。窗口默认按**剩余**显示百分比，可在设置里切换为**已用**；低额度告警始终按剩余 ≤ 20% 判定。不想要的窗口在设置里单独关掉即可，简洁模式不会替你隐藏它们。
...
**信息显示方式** —— 选择**简洁**或**完整**，选择会自动保存。两者只差一处：完整模式额外保留 DSH 原生统计行，简洁模式把它收起。字段开关不受影响，切来切去不会丢配置。

**字段与配色** —— 每个字段一个开关、一个颜色，分三组。关掉哪个字段，信息栏就不再显示它——**简洁与完整完全一致**。三组默认都收起，想看哪组点哪组（在搜索框里打字会把三组一起展开）。

- **原生信息** —— DSH 原生统计栏本来就有的字段，归在原生行里；只有完整模式才看得到这一行。
- **插件信息** —— 信息栏新增的全部内容：服务商与模型、订阅、花费、余额、定价与额度，以及接管过来的上下文圆环。简洁与完整都显示。
- **提醒信息** —— 更新与失败短标记，以及「刷新失败」这类一次性提醒。两种模式都显示，每条单独开关；只要当下真的有事要提醒就出现，与模式无关。

上下文圆环固定在主行最右端，而主行在两种模式下都在，所以它始终可见。
