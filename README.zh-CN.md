...
信息栏有两种显示方式，点击即可切换：**简洁**只保留服务商、模型和一项核心账户信息——余额、最短的有效额度窗口，或本计费周期花费；**完整**显示所有已启用细节，并保留 DSH 原生统计行。两种方式都跟随 DSH 的深色/浅色主题。
...
**字段与配色** —— 每个字段一个开关、一个颜色，分两组。关掉哪个字段，信息栏就不再显示它。插件信息默认展开；原生统计只在完整模式出现。
...
插件会自己检查更新：启动时查一次 npm，之后最多每 15 分钟复查一次。更新方式在插件设置页的**版本与更新**里二选一：

- **全自动更新**（默认）：发现新版本就自动下载、校验、替换好，你只需要重启一次 DSH 就生效。
- **手动更新**：只检查、不下载，你点「检查更新」时才执行。

替换前先校验文件完整性，任何一步没过就整批回滚，不会把正在用的版本弄坏；设置页里也随时可以点「回滚到上一版」。整个过程只改插件自己的文件，不碰依赖树，也不改 profile 的清单。

需要你动手时，信息栏会出现两枚短标记：**重启生效**（新版已装好，等重启）和**更新失败**（上次自动更新没成功）。两枚标记各自可以在「提醒信息」里关掉，平时不占位。
